...
#if defined(SERVER_LIB_PLATFORM_LINUX)
    if (impl::get_thread_id(thread_info) == impl::get_thread_id(impl::s_main_thread_info))
        impl::set_thread_main(thread_info);
#endif
}
...
void logger::add_boost_log_destination(const SinkTypePtr& sink, const std::string& time_format)
{
    ...
    auto boost_write = [pl = this->_boost_impl->get(), to_boost_level](const log_message& msg, int details_filter) {
        BOOST_LOG_SEV(*pl, to_boost_level(msg.context.lv))
            ...
            << boost::log::add_value("File", (~details_filter & static_cast<int>(logger::details::without_source_code)) ? msg.context.file.c_str() : "")
            << boost::log::add_value("ThreadId", (~details_filter & static_cast<int>(logger::details::without_thread_info)) ? static_cast<unsigned long>(impl::get_thread_id(msg.context.thread_info)) : 0)
            << boost::log::add_value("ThreadName", (~details_filter & static_cast<int>(logger::details::without_thread_info)) ? impl::get_thread_name(msg.context.thread_info) : "")
            << msg.message.str()
#if defined(SERVER_LIB_PLATFORM_WINDOWS)
